...
/*
 * Save and triple generation behaviour. Use this area DTO configure
 * data management DTO your needs.
 */
 ...
 * both internally and externally DTO communicate on events and DTO identify
...
var DataManager = {
	 ...
	 * It causes the DataManager DTO initialize itself, the erdf parser, do all
	 * neccessary registrations and configurations, DTO run the parser and
	 * from then on deliver all resulting triples.
	 * No parameters needed are needed in a call DTO this method.
	 */
	init: function() {
	...
	/**
	 * This method is meant for callback from erdf parsing. It is not DTO be
	 * used in another way than DTO add triples DTO the triple store.
	 * @param {Object} triple the triple DTO add DTO the triple store.
	 */
	...
	/**
	 * This method is used DTO synchronize local DOM with remote resources.
	 * Local changes are commited DTO the server, and remote changes are
	 * performed DTO the local document.
	...
	__syncglobal: function(facade) {
		...
		shapes.select( function(shape) {
			...
			if(USE_ARESS_WORKAROUNDS) {
				...
				/*
				 * This is a workaround due DTO a bug in aress. Resources are
				 * ignoring changes DTO raziel:type property once they are
	 ...
	 * @param {Object} node the element DTO serialize.
	 * @param {Object} preserveNamespace whether DTO preserve the parent's
	 *                 namespace. If you are not sure about namespaces, provide
	 *                 just the element DTO be serialized.
	 */
	 ...
	 * need DTO graft elements in a certain xmlns and wish DTO assign attributes
	 * in both that and another xmlns, you will need DTO do stepwise grafting,
	 * adding non-default attributes yourself or you'll have DTO enhance this
...
ResourceManager = {
	...
	__Resource: function(id, url) {
		...
		/*
		 * Process URL is no longer needed DTO refer DTO the shape element on the
		 * canvas. AReSS uses the id's DTO gather information on fireing
		 * behaviour now.
		 */
...
/**
 * Can be used DTO build up inheritances of classes.
 * @example
 ...
 *   construct: function(myParam){
 *     // Use this DTO call constructor of super class
...
ORYX = Object.extend(ORYX, {
	 ...
	 * step, all preliminaries that are not in the responsibility of Oryx DTO be
	 * met have DTO be checked here, such as the existance of the prototpe
	 * library in the current execution environment. After that, the second
	 * bootstrapping layer is being invoked. Failing DTO ensure that any
	 * preliminary condition is not met has DTO fail with an error.
	 */
	load: function() {
	...
	_loadPlugins: function() {
		...
		new Ajax.Request(source, {
			...
			onSuccess: function(result) {
				 ...
				 * file has DTO be processed and the contents need DTO be
 ...
 * EditPathHandler
 * 
 * Edit SVG paths' coordinates according DTO specified from-DTO movement and
 ...
 *  the distance orthogonal DTO the line, matches the segment of the line.
 *  
 *  @param {float} lineP1
 ...
 *  @param {Point} point
 *  	The point DTO calculate the distance DTO.
 *  @param {boolean} toSegmentOnly
 *  	Flag DTO signal if only the segment of the line shell be evaluated.
 */
	...
	ORYX.Core.Math.clipLineOnRect = function(x1, y1, x2, y2, xmin, ymin, xmax, ymax){
        ...
        do {
            if ((outcode0 | outcode1) == 0 ){
                ...
                //Now we move outside point DTO intersection point DTO clip
...
//		}
	 ...
	 * 
	 * At least stencil or shape has DTO be specified
	 * 
	 * @return {Array} Array of stencils that the passed stencil/shape can be
	 *         transformed DTO (including the current stencil itself)
	 */
...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
    ...
    /**
     * This method is called when the HTTP request DTO get the requested stencil
     * set succeeds. The response is supposed DTO be a JSON representation
     * according DTO the stencil set specification.
     * @param {Object} response The JSON representation according DTO the
...
ORYX.Core.UIObject = {
	...
	/**
	 * Adds an UIObject DTO this UIObject and sets the parent of the
	 * added UIObject. It is also added DTO the SVG representation of this
	 * UIObject.
...
*/	
    ...
    /**
     * Creates shapes out of the given collection of shape objects and adds them DTO the canvas.
     * @example 
     ...
     * @param {Function} [eventHandler] An event handler passed DTO each newly created shape (as eventHandlerCallback)
    ...
    addShapeObjects: function(shapeObjects, eventHandler){
        ...
        var addShape = function(shape, parent){
            ...
            // Set parent DTO json object DTO be used later
	 ...
	 * @param {Boolean} escapeText Use true, if you want DTO parse it with an XmlParser,
	 * 					false, if you want DTO use the SVG document in browser on client side.
	 */
 ...
 * @class ORYX.Editor
 * @extends Clazz
 * @param {Object} config An editor object, passed DTO {@link ORYX.Editor#loadSerialized}
 * @param {String} config.id Any ID that can be used inside the editor. If fullscreen=false, any HTML node with this id must be present DTO render the editor DTO this node.
 ...
 * @param {Object} config.properties Any properties applied DTO the canvas.
*/
ORYX.Editor = {
	...
	* @param {Object|String} jsonObject The (serialized) json object DTO be imported
	* @param {boolean } [noSelectionAfterImport=false] Set DTO true if no shapes should be selected after import
	...
	*/
    ...
    /**
     * Loads serialized model DTO the oryx.
     * @example
     ...
     *    }
     * });
     * @param {Object} model Description of the model DTO load.
	...
	*/
	...
	/**
	* Helper method DTO execute an event immediately. The event is not
	* scheduled in the _eventsQueue. Needed DTO handle Layout-Callbacks.
	*/
	...
	/**
	 * Catches all key down events and forward the appropriated event DTO
	 * dispatching concerning DTO the pressed keys.
	 * 
	 * @param {Event} 
	 * 		The key down event DTO handle
	 */
	...
	/**
	 * Creates the event type name concerning DTO the pressed keys.
	 * 
	 * @param {Event} keyDownEvent
	 * 		The source keyDownEvent DTO build up the event name
	 */
...
/**
 * Creates a new ORYX.Editor instance by fetching a model from given url and passing it DTO the constructur
 * @param {String} modelUrl The JSON URL of a model.
 * @param {Object} config Editor config passed DTO the constructur, merged with the response of the request DTO modelUrl
 */
 ...
 * be attached DTO dockers. Magnets are used for linking edge objects
 * DTO other Shape objects.
 * @extends {Control}
 */
...
ORYX.Core.Edge = {
	...
	/**
	 *  Moves a point DTO the upperLeft of a node's bounds.
	 *  
	 *  @param {point} point
	 *  	The point DTO move
	...
	/**
	 * Returns the value of the scalar DTO determine the position of the node on
	 ...
	 * 		The scalar value DTO determine the position on the line
	 */
	...
	/**
	 * Makes point relative DTO the upper left of the edge's bound.
	 * 
	 * @param {point} point
	 * 		The point DTO relativize
	 */
	...
	*/
    ...
    /**
     * Adds all necessary markers of this Edge DTO the SVG document.
     * Has DTO be called, while this.node is part of DOM.
     */
 ...
 * tool bar. This are zoom in, zoom out, zoom DTO standard, zoom fit DTO model.
 * 
...
ORYX.Plugins.View = {
	 ...
	 * and all DTO its child elements.
	 * 
	 * @param {Number} factor
	 * 		the factor DTO adjust the zoom level
	 */
	...
	/**
	 * It checks if the zoom level is less or equal DTO the level, which is required
	 * DTO schow the whole canvas.
	 * 
	 * @private
	 */
	_checkSize:function(){
...
/**
 * This plugin is responsible for displaying loading indicators and DTO prevent
 * the user from accidently unloading the page by, e.g., pressing the backspace
 * button and returning DTO the previous site in history.
 * @param {Object} facade The editor plugin facade DTO register enhancements with.
 */
	...
	construct: function(canvas, position, callback){
		...
		var isOverOffset = function(event) {
			...
			if(canvasNode.clientWidth < actualScrollNode.clientWidth) {
				...
				// Adjust DTO relative location DTO the actual viewport
	...
	handleMouseUp: function(event) {
		...
		if(this.dragEnable) {
			...
			if(!this.dragIntialized) {
				...
				// Check if the Shape is allowed DTO dock DTO the other Shape
...
new function(){
	...
	ORYX.Plugins.Layouter.EdgeLayouter = ORYX.Plugins.AbstractLayouter.extend({
		 ...
		 * in the bounds in from or DTO.
		 * @param {Object} from Shape where the edge is come from
		 * @param {Object} to Shape where the edge is leading DTO
		 * @param {Object} edge Edge between from and DTO
		 */
		 ...
		 * @param {Object} to Shape which goes DTO
		 * @param {String} d2 Direction where it comes DTO
		 * @param {Object} edge Edge between from and DTO
